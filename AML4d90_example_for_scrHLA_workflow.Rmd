...
write(top_alleles[["top_alleles"]], file.path(dirs_path, paste0(substr(samples[1],1,nchar(samples[1])-3), "_guided", "_top_alleles.csv")))
...
## Important Note ##
# in older versions of `scrHLAtag`, the CB and UMI tags are missing from the scrHLAtag output BAM files
# there is a script to "add those back" to the original bam files
# This will be extremely useful later for merging bams and/or subsetting bams based on CBs

## find all the bams output from scrHLAtag
search_dir="${1:-/fh/fast//furlan_s/user/skanaan/scrHLAtyping/WorkFlowTest_AML401}" 
# use mapfile to read the results of find into an array
mapfile -t bam_paths < <(
  find "$search_dir" -type f \( \
    -name "Aligned_mm2_sorted_mRNA.bam" \
  \)
)
# bam_paths now holds all matching files
echo "Found ${#bam_paths[@]} BAM(s):"
for bam in "${bam_paths[@]}"; do
  echo "  $bam"
done

# you can loop through all or part of the array
start_index=0 # first index is 0 because array indices are zero-based
end_index=$((${#bam_paths[@]} - 1)) # -1 because array indices are zero-based

for ((i=start_index; i<=end_index; i++)); do
    export ibam="${bam_paths[i]}"
    export in_dir=$(dirname "$ibam")
    cd "$in_dir"
    sbatch -n 1 -c 1 -p campus-new --mem-per-cpu=21000MB \
      --wrap="~/develop/scrHLAtag/target/release/addback -b $ibam \
      && mv out.bam $ibam \
      && samtools index $ibam"
done
squeue -u skanaan

# after "mRNA", do the same for the "gene" output
search_dir="${1:-/fh/fast//furlan_s/user/skanaan/scrHLAtyping/WorkFlowTest_AML401}" 
# use mapfile to read the results of find into an array
mapfile -t bam_paths < <(
  find "$search_dir" -type f \( \
    -name "Aligned_mm2_sorted_gene.bam" \
  \)
)
# bam_paths now holds all matching files
echo "Found ${#bam_paths[@]} BAM(s):"
