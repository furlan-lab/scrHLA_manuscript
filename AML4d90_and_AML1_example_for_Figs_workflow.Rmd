...
seu@active.ident
levels(seu)
seu@active.ident <- as.factor(as.character(seu$malignant))
mismatch_patterns <- lapply(1:length(levels(seu)), function(mal) {
  ...
  mismatch_data <- pbmcapply::pbmclapply(1:length(levels(reads@seqnames@values)), function(x){
    ...
    pp <- PileupParam(
      ...
      min_base_quality = 0, # =0 so nothing blows up
    ...
    ref_seq <- BSgenome.Hsapiens.scrHLAtyping.imgtHLA[[ctg]]
    ...
    pu2$type[is.na(pu2$type)] <- "ambiguous"
    # aligners 'anchor' insertions to the preceding ref base, so match and ins may happen at same position. to avoid double count:
    ins_by_pos <- pu2[which(pu2$type == "insertion"),]
    pu2$ins_count <- ins_by_pos$count[match(pu2$pos, ins_by_pos$pos)]
    pu2$ins_count[is.na(pu2$ins_count)] <- 0
    pu2$count_adj <- pu2$count - as.numeric(pu2$type == "_match") * pu2$ins_count #TRUE coerces to 1 and FALSE to 0
    # compute rates:
    rate_df <- pu2 %>%
      group_by(pos, type) %>%
      summarise(count_adj = sum(count_adj), .groups="drop") %>%
      group_by(pos) %>%
      mutate(depth = sum(count_adj), rate = count_adj / depth) %>%
      ungroup()
...
### Details in a separate notebook at: https://github.com/furlan-lab/scrHLA_manuscript/blob/main/AML1_Bash_and_R_analysis_shortread_mutations.Rmd ###
...
### Details in a separate notebook at: https://github.com/furlan-lab/scrHLA_manuscript/blob/main/AML4d90_example_for_GeneFusion_workflow.Rmd ###
...
### Details in a separate notebook at: https://github.com/furlan-lab/scrHLA_manuscript/blob/main/insilico_mixes_withFigs.Rmd ###
