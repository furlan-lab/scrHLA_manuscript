...
dirnames <- c("AML_F_5", "AML_E_5", "TN_BM", "TN_34")               # this is how the 5' samples were organized in the directories
...
# dirs_path <- "/fh/fast/furlan_s/grp/data/targ_reseq/01_scrHLA_project/231020_longreadTSO_HLA_MDS_DB_LA_MO_EWS/3_C01" # 3' files
dirs<-list.dirs(path=dirs_path, full.names = T, recursive = F)
dirs<- lapply(dirs, dir, pattern = "^guided3", recursive = F, full.names = T) %>% unlist
dirnames <- c("AML_F_5", "AML_E_5", "TN_BM", "TN_34") # this is how the 5' samples were organized in the directories
# dirnames <- c("Pre_Ifng_4", "Post_Ifng_4", "AML_E_3", "MO1_IP_1") # this is how the 3' samples were organized in the directories
...
## Run HLA_Matrix() 
hla <- HLA_Matrix(reads = cts[["mRNA"]], seu = seu, 
                  CB_rev_com = F,    # TRUE for 3prime 10x on pacbio
                  hla_recip = rp,
                  hla_donor = dn,
                  return_stats = F,
                  parallelize = F)
if (class(hla) == "Assay") {suppressWarnings(seu[["HLA"]] <- hla)} else if (class(hla) == "list") {suppressWarnings(seu[["HLA"]] <- hla[["matrix"]])}
...
```{r}
## a list of pre-processed and saved Seurat objects, in our example just one:
## AML4 day 90 post-tx
rds <- c(
  "220926_seuratObj_TN1_celltype_genotype.RDS"
  )          
names(rds) <- c("AML_401")
samp <- 1
...
dirnames <- c("TN_BM", "TN_34") # this is how the samples were organized in the directories

## Load the counts files
cts <- HLA_load(directories = dirs, dir_names = dirnames, seu = seu)

## define the recipient-specific and donor-specific HLA alleles if known. For example:
rp = c("A*24:02:01", "B*48:01:01", "C*08:06", "DMA*01:02:01", "DPA1*01:03:01", "DPB1*04:02:01", "DQA1*03:03:01", "DQB1*03:01:01", "DRA*01:01:01", "DRB1*04:01:01", "DRB4*01:03:02", "E*01:03:02", "F*01:01:01")
...
perstep_stats <- data.table::fread(file.path(DATA_DIR, paste0("TN_per_step_stats-iter4.csv")), header = T, sep=",")
umi_dupl <- data.table::fread(file.path(DATA_DIR, paste0("TN_umi_dupl-iter4.csv")), header = T, sep=",")
...
## testing
seu[["HLA"]]@data[1:3,1:3]
seu[["HLA"]]@counts[1:3,1:3]
row.names(seu[["HLA"]])

## visualizing
DefaultAssay(seu) <- "HLA"
hla_gene <- "A"
select_hla <- row.names(seu[["HLA"]])[grep(paste0("^", hla_gene), row.names(seu[["HLA"]]))]
...
soup <- 1 # souporcell 1 or 2
{
  ...
  long_df <- cbind(
    seu_sub[["HLA"]]@counts[select_hla, , drop = F] %>% as.data.frame() %>% t(),
    seu_sub[["RNA"]]@counts[select_rna, , drop = F] %>% as.data.frame() %>% t()
...
## let's create a long dataframe for ADT vs. Normalized Gene Exp values per celltype. 
{
  ...
  for (a in seq_along(hla_gene)) {
    select_hla <- row.names(seu[["HLA"]])[grep(paste0("^", hla_gene[a]), row.names(seu[["HLA"]]))]
    ...
    for (soup in seq_along(sort(unique(seu@meta.data$geno)))) {
      ...
      long_df <- cbind(
        seu_sub[["HLA"]]@counts[select_hla, , drop = F] %>% as.data.frame() %>% t(),
        seu_sub[["ADT"]]@data[select_adt, , drop = F] %>% as.data.frame() %>% t(),
...
#------ Fig.4c, Suppl.Fig.S11c ------#
DimPlot_scCustom(seu, group.by = "malignant", reduction = "umap", pt.size=0.4, colors_use = c("grey85", "red", "violetred4", "black"), order = c("Rcp_Malignant","Rcp_Healthy", "Don_Healthy")) +scale_color_manual(values= c("grey85", "red", "violetred4", "black"), drop=F) # drop=F to show a level in legend even if it is unused in the plot (4in x 6in)
...
{ # run this chunk to generate the figure
  av.expr1 <- assay(muscat::aggregateData(as.SingleCellExperiment(seu), by = c("ident"), fun = "mean")) %>% as.data.frame() %>% suppressWarnings() 
  av.expr2 <- assay(muscat::aggregateData(as.SingleCellExperiment(seu), by = c("ident"), fun = "prop.detected")) %>% as.data.frame() %>% suppressWarnings()
  ...
  fun.ratio <- function(df) {
    if (any(grepl("shared", df$AlleleOrigin))) {
      dh <- df$Don_Healthy[grepl("shared", df$AlleleOrigin)]
      rh <- df$Rcp_Healthy[grepl("shared", df$AlleleOrigin)]
      rm <- df$Rcp_Malignant[grepl("shared", df$AlleleOrigin)]
      # get the ratios over the shared allele value
      df$Don_Healthy <- df$Don_Healthy/dh
      df$Rcp_Healthy <- df$Rcp_Healthy/rh
      df$Rcp_Malignant <- df$Rcp_Malignant/rm
    } else {
      df$Don_Healthy <- NA
      df$Rcp_Healthy <- NA
      df$Rcp_Malignant <- NA
  ...
  av.expr.mlt1 <- reshape2::melt(av.expr1, id.vars = "AlleleOrigin", measure.vars = colnames(av.expr1[,c(1:3)]), variable.name = "cell_grp",)
  av.expr.mlt2 <- reshape2::melt(av.expr2, id.vars = "AlleleOrigin", measure.vars = colnames(av.expr2[,c(1:3)]), variable.name = "cell_grp",)
  av.expr.mlt3 <- reshape2::melt(av.expr3, id.vars = "AlleleOrigin", measure.vars = colnames(av.expr3[,c(1:3)]), variable.name = "cell_grp",)
  av.expr.mlt4 <- reshape2::melt(av.expr4, id.vars = "AlleleOrigin", measure.vars = colnames(av.expr4[,c(1:3)]), variable.name = "cell_grp",)
  ...
  av.expr.mlt$under_press_MeanNormExprRatioOverShared <- ifelse(
    (grepl("don", av.expr.mlt$AlleleOrigin, ignore.case = T) & grepl("don", av.expr.mlt$cell_grp, ignore.case = T)) | 
      (grepl("recip", av.expr.mlt$AlleleOrigin, ignore.case = T) & grepl("rcp", av.expr.mlt$cell_grp, ignore.case = T)), av.expr.mlt$MeanNormExprRatioOverShared, NA
  )# identify under pressure alleles (the non-shared allele)
  av.expr.mlt$under_press_PropExprRatioOverShared <- ifelse(
    (grepl("don", av.expr.mlt$AlleleOrigin, ignore.case = T) & grepl("don", av.expr.mlt$cell_grp, ignore.case = T)) | 
      (grepl("recip", av.expr.mlt$AlleleOrigin, ignore.case = T) & grepl("rcp", av.expr.mlt$cell_grp, ignore.case = T)), av.expr.mlt$PropExprRatioOverShared, NA
  )
...
Stacked_VlnPlot(seu, features = c(row.names(seu)[29], row.names(seu)[28]), #DRB1
                x_lab_rotate = TRUE, split.by = "malignant", colors_use = pal4, plot_legend =T)

#------ Fig.4e, Suppl.Fig.S11d ------#
{
  ...
  rm(old.lvl)
  # subset the seu 
  seu_malig <- seu$malignant %>% unique() %>% sort()
  seus <- c()
  ...
  seus <- lapply(seus, function(seu){
    cts <- seu[["HLA"]]@counts %>% as.data.frame()
  })
  ...
  seus <- lapply(seus, function(cts) {
    cts$underprss <- ifelse(
      (grepl("don", cts$AlleleOrigin, ignore.case = T) & grepl("don", cts$seu_subset, ignore.case = T)) | 
        (grepl("recip", cts$AlleleOrigin, ignore.case = T) & grepl("rcp", cts$seu_subset, ignore.case = T)), "underpressure", ifelse(grepl("shared", cts$AlleleOrigin, ignore.case = T), "shared/hmz", NA)
    )
...
## Suppl.Fig.S2 and Suppl.Fig.S4 
# Molecular Swap

```{r}

#------ Suppl.Fig.S2b, S2c, S4b, or S4c------#
## map with the genotypes found in the seurat object
ctsu <- cts[["mRNA"]]
special <- "[_*|?.+$^]"
ctsu$gene0 <- gsub(special, "-", ctsu$gene)
...
fasta <- "HLA_of_TN1.fa"          #change every time
...
# example of clear molecular swap between HLA-A and HLA-B in TN1
...
```{r}
## load pre-aggregated data, previously collected using 'HLA_Matrix()' with arg 'return_stats = TRUE'
perstep_stats <- data.table::fread(file.path(DATA_DIR, paste0("DB_per_step_stats-iter4.csv")), header = T, sep=",")
umi_dupl <- data.table::fread(file.path(DATA_DIR, paste0("DB_umi_dupl-iter4.csv")), header = T, sep=",")
...
#------ Suppl.Fig.S10b ------#
## construct plots p1, p2, p3, p4, p5, and p6 with all 3 time-point captures of TN (days 90, 140, 163) 
## and for each souporcell genotype

soup <- 1 # soup 1 and 2 for each TN timepoints, loaded separately into a new 'seu' object
seu_sub <- seu[ , rownames(seu@meta.data)[seu@meta.data$geno == sort(unique(seu@meta.data$geno))[soup]]]
...
# saveRDS(plot, file.path(DATA_DIR, "celltype_composition_TN123.RDS"))
plot <- readRDS(file.path(DATA_DIR, "celltype_composition_TN123.RDS"))
DimPlot(seu, group.by = "celltype", cols = bmcolsl, split.by = "geno") /plot # 5x7 in then scale down to 75%

#------ Suppl.Fig.S10c ------#
## it looks like 4 major cell types constitute malignant cells. Let's set them apart
## and check if they manifest 'stemness' or 'immortal' properties

seu$malig <- NA
seu$malig <- ifelse(seu$geno == "Recipient" & seu$celltype %in% c("01_HSC", "02_Early_Erythroid", "04_Myeloid_Progenitor", "08_CD14_Monocyte"), "malignant", "healthy")
DimPlot_scCustom(seu, group.by = "malig", reduction = "umap", pt.size=0.4, colors_use = c("grey85", "red", "violetred4", "black"))
seu$malignant <- ifelse(seu$geno == "Recipient", "Rcp_Healthy", "Don_Healthy")
seu$malignant <- ifelse(seu$malig == "malignant", "Rcp_Malignant", seu$malignant)
Idents(seu)<-seu$malignant
old.lvl<-levels(Idents(seu)) %>% sort()
...
ra1 <- ref_align[ref_align@ranges@NAMES %like% "C\\|04:09N"]
